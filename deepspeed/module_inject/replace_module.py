...
def replace_transformer_layer(orig_layer_impl,
    ...
    def replace_wo_policy(module, all_reduce_linears, device, async_collective=False):
        def _replace(child, name, conv_linear_layer):
            ...
            if name in all_reduce_linears:
                ...
                return LinearAllreduce(data, child.bias if child.bias is None else \
                            torch.nn.parameter.Parameter(move(new_bias, device)), mp_group, async_collective=async_collective)
            else:
                new_weight = torch.empty((
                    (weight_shape[1] if conv_linear_layer else weight_shape[0]) //
                    mp_size,
    ...
    def replace_fn(child, _policy, layer_id=0):
        if training:
            ...
            if replace_with_kernel_inject:
                ...
                new_module = replace_wo_policy(child, _policy, device=device, async_collective=async_collectives)
    ...
    if checkpoint_dict is not None:
        ...
        if ckpt_type == 'pp':
            ...
            for i in range(len(checkpoint)):
                ...
                load_model_with_checkpoint(replaced_module, sd, mp_replace, ckpt_type, device=device)
        else:
