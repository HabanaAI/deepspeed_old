...
from .layers import LinearLayer, LinearAllreduce, Normalize, EmbeddingLayer
import torch


def load_model_with_checkpoint(r_module, sd, mp_replace, ckpt_type, rank=0, device=None):
    error_msgs = []
    ...
    def load(module, prefix):
        ...
        if len(list(module.parameters())) > 0 and list(
            ...
            if prefix + 'bias' in sd.keys():
                module.bias = mp_replace.copy(module.bias, sd[prefix + 'bias'])
            ...
            if child.__class__ in layer_policies:
                ...
                if len(child_params) > 0 and (child_params[0].numel() == 0
                    ...
                    if child.__class__ is nn.LayerNorm:
                        ...
                        child = LinearLayer(weight=child.weight, bias=child.bias,device=device)
                        setattr(module, name, child)
                    else:
                        ds_id = None
