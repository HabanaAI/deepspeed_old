...
def see_memory_usage(message, force=False):
    ...
    if not torch.cuda.is_available():
        logger.info(message)
        logger.info("Memory stats not supported for non cuda device")
        return
        ...
        for shard_id in range(num_shards):
            ...
            with torch.no_grad():
                ...
                dist.all_gather(shard_list,
                                ...
                                async_op=True if device.type=='hpu' else False)
