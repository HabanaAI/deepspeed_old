...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def __init__(self,
                 ...
                 use_hpu=False,
                 no_cuda=False,
                 hpu_use_async_collectives=False):
        ...
        self.async_collectives = self.device.type == "hpu" and hpu_use_async_collectives
    ...
    def gradient_reduction_w_predivide(self, tensor):
        ...
        if self.postscale_gradients:
            ...
            dist.all_reduce(tensor_to_allreduce, group=self.dp_process_group,
                            async_op = self.async_collectives)
    ...
    def complete_grad_norm_calculation_for_cpu_offload(self, params):
        ...
        total_norm_cuda = torch.cuda.FloatTensor([float(total_norm)]).to(self.device)
    ...
    def get_grad_norm_direct(self, gradients, params, norm_type=2):
        ...
        if norm_type == inf:
            ...
            for g, p in zip(gradients, params):
                ...
                if hasattr(p, PIPE_REPLICATED) and p.ds_pipe_replicated:
                    continue
                if is_model_parallel_parameter(p) or (self.model_parallel_rank == 0):
                    all_norms.append(torch.norm(g.data.double().detach(), norm_type).to(self.device))
            total_norm = torch.stack(all_norms).square().sum()
            # Sum across all model parallel Device.
            total_norm_cuda = total_norm.float()
            dist.all_reduce(total_norm_cuda,
                            op=dist.ReduceOp.SUM,
                            group=self.dp_process_group,
                            async_op = self.async_collectives)
        ...
        if total_norm.isinf():
            total_norm = torch.tensor(-1, device=self.device).float()
    ...
    def step(self, closure=None):
        ...
        all_gather_dp_groups(
            ...
            async_collective=self.async_collectives)
