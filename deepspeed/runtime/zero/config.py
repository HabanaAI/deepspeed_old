...
class DeepSpeedZeroConfig(DeepSpeedConfigModel):
    ...
    hpu_use_async_collectives: bool = False
    ...
    @validator("hpu_use_async_collectives")
    def hpu_use_async_collectives_valid(cls, field_value, values):
        assert (
            "stage" in values
        ), "DeepSpeedZeroConfig: 'stage' must be defined before 'hpu_use_async_collectives'"
        if field_value is True:
            assert (
                values["stage"] == ZeroStageEnum.optimizer_states or values["stage"] == ZeroStageEnum.gradients
            ), "DeepSpeedZeroConfig: 'hpu_use_async_collectives' only valid for 'stage' = 1 or 2"
        return field_value