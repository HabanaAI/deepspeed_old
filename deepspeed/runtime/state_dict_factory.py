...
class SDLoaderBase(ABC):
    def __init__(self, ckpt_list, version, checkpoint_engine, device=None):
        ...
        if device == "hpu" or device == torch.device("hpu"):
            self.map_location=torch.device("cpu")
        elif torch.cuda.is_available():
            #TODO: change to lambda storage: 'cpu' if str(storage).startswith("hpu") else storage
            #and see whether it works (SW-113718)
            self.map_location=lambda storage, \
                        loc: str(storage).startswith("hpu") and "cpu" or storage
        else:
            self.map_location=lambda storage, \
                    loc: storage
