...
class InferenceEngine(Module):
    ...
    def __init__(self,
        ...
        self.module = model
        if config:
            self.use_hpu = config.device == "hpu" if hasattr(config, "device") else False
            # True by default for use_hpu
            self.async_collectives = config.async_collectives and self.use_hpu if hasattr(config, "async_collectives") else self.use_hpu
        else:
            self.async_collectives = False
            self.use_hpu = False
        ...
        if self.checkpoint and not replace_with_kernel_inject and not self.use_hpu:
        ...
        if self.injection_dict:
            for client_module, injection_policy in self.injection_dict.items():
                self._apply_injection_policy(
                    ...
                    moe_type,
                    training_mp_size,
                    #TODO:[SW-111290] check why we need to pass checkpoint always
                    self.checkpoint,
        ...
        if self.enable_cuda_graph and self.use_hpu:
    ...
    def _load_checkpoint(self, load_dir, load_module_strict=True, tag=None):
        ...
        if os.path.isdir(load_dir):
            ...
            sd_loader = SDLoaderFactory.get_sd_loader_json(load_dir, device=self.module.device)
    ...
    def forward(self, *inputs, **kwargs):
        ...
        """
        if self.enable_cuda_graph and not self.use_hpu:
